...
export class ContactsDetailsComponent implements OnInit, OnDestroy
{
    ...
    sendToBeneficiaries: boolean = false;
    mainSendBtn: boolean = true;
    ...
    SendMoneyBeneficiariesForm: UntypedFormGroup;
    newAccountSendMoneyForm: UntypedFormGroup;
    contacts: Contact[];
    countries: Country[];
    ...
    ngOnInit(): void
    {
        // this.newAccount = this._commonService.newAccount;
        if(this._commonService.newAccount){
            console.log(this._commonService.newAccount,'lslsl');
            this.newAccount = this._commonService.newAccount;
            this.editMode = false;
        }
        // Open the drawer
        ...
        this.newAccountSendMoneyForm = this._formBuilder.group({
        ...
        this.SendMoneyBeneficiariesForm = this._formBuilder.group({
            amount      : ['', [Validators.required]],
            reference   : ['', [Validators.required]]
        });
        // Get the contacts
        ...
        // Get the contact
        this._accountService.getBeneficiariesID$
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((contact: Contact) => {
                this.mainSendBtn = true;
     ...
     * @param newAccount
     */
     toggleNewAccount(newAccount: boolean | null = null): void
     {
         if ( newAccount === null )
         {
    ...
    updateContact(): void
    {
       ...
       * Update the contact
       */
       sendMoneyToNewAddress(): void
       {
        this._userService.account$.subscribe((account) =>{
            const sendMonetDetails = this.SendMoneyBeneficiariesForm.getRawValue();
            const data ={
                'amount': sendMonetDetails.amount,
                'currency': sendMonetDetails.currency,
                'destination': {
                    'iban': sendMonetDetails.iban,
                    'name': sendMonetDetails.name,
                    'type': sendMonetDetails.type
                },
                'reference': sendMonetDetails.reference,
                'sourceAccountId': account[0].ledgerId,
            };
            this._contactsService.newAccountDetails = data;
            // Open the dialog
    ...
    sliderClose(): void {
        this._contactsListComponent.openSlider = false;
    }
    // snedMoneyBeneficiarie(): void {
    //     // this._contactsService.newAccountDetails;
    //     // Open the dialog
    //     const dialogRef = this._matDialog.open(WalletTransationComponent);
    ...
    //     });
    // }
    sendMoneyBeneficiariesAddress(contact): void {
        if(contact.benModule === 'MODULR'){
            this._userService.account$.subscribe((account) =>{
                const sendMonetDetails = this.SendMoneyBeneficiariesForm.getRawValue();
                const data ={
                    'amount': sendMonetDetails.amount,
                    'currency': contact.destinationIdentifier.currency,
                    'destination': {
                        'id': contact.benId,
                        'iban': contact.destinationIdentifier.iban,
                        'name': contact.name,
                        'type': contact.destinationIdentifier.type
                    },
                    'reference': sendMonetDetails.reference,
                    'sourceAccountId': account[0].ledgerId,
        ...
        }else  if(contact.benModule === 'WISE'){
            let currency = '';
            if(contact.destinationIdentifier.currency === 'SGD'){
                currency = 'S$';
            }
            const sendMonetDetails = this.SendMoneyBeneficiariesForm.getRawValue();
            const data ={
                'name': contact.name,
                'sourceAmount': sendMonetDetails.amount,
                'sourceCurrency': contact.destinationIdentifier.currency,
                'targetCurrency': contact.destinationIdentifier.currency,
                'reference': sendMonetDetails.reference,
                'benId' : contact.benId,
                'benModule': 'WISE',
                'currency':currency
            };
            this._accountService.sendWiseMoneyData = data;
            const dialogRef = this._matDialog.open(WalletTransationComponent);
            dialogRef.afterClosed().subscribe((result) => {
                console.log('Compose dialog was closed!');
                this._router.navigate(['../'], {relativeTo: this._activatedRoute});
