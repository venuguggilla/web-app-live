...
import { InventoryProduct } from '../../../ecommerce/inventory/inventory.types';
import { InventoryBrand, InventoryCategory, InventoryPagination, InventoryTag, InventoryVendor } from './tx-alert.types';
 ...
 */
 ...
 blockUnblockaAccount(account)
 {
  ...
  Observable<{data: any}>
    ...
    url = `${url}/${userId}/wallet`;
    return this._httpClient.get<{data: any}>(url);
    }

// eslint-disable-next-line @typescript-eslint/adjacent-overload-signatures
  getWalletsTrasaction22(userId):
  Observable<{data: any}>
    {
    let url = this._commonService.baseUrl + this._apiEndpoint.userData.user;
    url = `${url}/${userId}/transactions`;
    return this._httpClient.post<{data: any}>(url, {
        page:0,
        pageSize:10,
        walletId:'1566279497682685954'});
    }
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
